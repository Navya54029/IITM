...
def dashboard(user_id):
        ...
        response=requests.get(f"http://127.0.0.1:5000/v1/api/user/{user_id}")
        user_data1=json.loads(response.text)
        print(user_data1)
        if user_data1['logout_time'] is not None:
            user_data1['logout_time']=user_data1['logout_time'].split('.')[0]
        ...
        if responsetrackers.status_code == 404:
            print("Inside if")
            return render_template("dashboard.html",user_data=user_data1)
        else:
            ...
            return render_template("dashboard.html",user_data=user_data1,user_tracker_details=user_tracker_details,last_tracked=dates,name=current_user.user_name)
